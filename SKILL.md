...
32. When the user wants a full morning overview instead of a filing pass, prefer `daily-briefing`. Treat it as a read-only AI-gated workflow: the first run should emit a snapshot plus plan template, not a built-in bucketed answer.
33. The AI should decide which tasks are actually actionable today and which briefing buckets matter. Do not let Python heuristics choose the final morning queue for `daily-briefing`.
34. The desired `daily-briefing` output is a reviewed JSON plan plus a rendered command center: a short overview, a short focus line, highlighted tasks with direct links, and explicit `ask_user` questions for anything ambiguous.
